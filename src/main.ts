...
import { gsArgs, gsExec, gsShortArgs } from "./gs-cmd.ts"
import { getArguments } from "./getArguments.ts";
import { ArgumentsType, readArgs, retrieveArgs } from "./retrieveArgs.ts";
...
for await (const dirEntry of Deno.readDir(args.directory)) {
  ...
  const cmdArgs = gsShortArgs.replace('{{input}}', join(args.directory, dirEntry.name)).replace('{{output}}', join(args.outDirectory, dirEntry.name));
